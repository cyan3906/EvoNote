...
For production, set `APP_ENV=production` or `ENFORCE_SECURE_CONFIG=true`, replace the default password/token, and configure `CORS_ALLOWED_ORIGINS`. `AUTH_PASSWORD_HASH` can be used instead of a plain password; supported hashes use the `pbkdf2_sha256$iterations$salt$digest` format generated by `app.core.security.hash_password`.
...
- `GET /api/evolution/notes/{note_id}/scan/status`: read the persisted background scan job status.
...
Scan jobs are persisted in SQLite, retried up to `EVOLUTION_JOB_MAX_ATTEMPTS`, and unfinished queued/running jobs are recovered when the API starts.
